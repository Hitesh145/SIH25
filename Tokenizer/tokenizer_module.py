...
def tokenize_log(entry):
    ...
    tokens = tokenizer(flat, return_tensors="pt", truncation=True, padding=True)
    return {
...
if __name__ == "__main__":
    ...
    with open("logs/parsed_logs.json", "r") as f:
        parsed_logs = json.load(f)

    tokenized_output = process_logs(parsed_logs)

    # Save tokenized logs
    with open("logs/tokenized_logs.json", "w") as f:
        json.dump(tokenized_output, f, indent=2)
